...
namespace emoc {
	...
	Algorithm::Algorithm(Problem *problem, int thread_id):problem_(problem), thread_id_(thread_id)
	{
        ...
        std::cout<<"<info> Algorithm::Algorithm"<<std::endl;
	}
	...
	void Algorithm::TrackPopulation(int generation, double *reference_point)
	{
		RecordPop(g_GlobalSettings->run_id_, generation, g_GlobalSettings, reference_point);
	}
